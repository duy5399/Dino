...
public class MortScript : MyPlayer
{
    private GameObject fireEffect;
    public Sprite mortWon;
    public Sprite fireImgSkill;
    ...
    void Start()
    {
        ...
        if (photonView.IsMine)  //PhotonView: đối tượng trên mạng (xác định bằng viewID)
        {
            ...
            skillBtn.GetComponent<Image>().sprite = fireImgSkill;
        }
        else
        {
    ...
    public void Skill()
    {
        ...
        {
            Debug.Log("Skill");
            fireEffect = PhotonNetwork.Instantiate(effectPrefab.name, effectPosition.position, Quaternion.identity);
        ...
        if (cooldown)
        {
            if (skillDelay > 0)
            {
                if (skillDelay > 2 && fireEffect != null)
                {
                    //fireEffect.GetComponent<FireScript>().updatePosition(gameObject.transform.position);
                    fireEffect.GetComponent<PhotonView>().RPC("UpdatePosition", RpcTarget.AllBuffered, gameObject.transform.position);
                    //if (sr.flipX == true)
                    //{
                    //    fireEffect.GetComponent<SpriteRenderer>().flipX = true;
                    //}
                }
                //else
                //{
                //    //Destroy(fireEffect);
                //    fireEffect.GetComponent<PhotonView>().RPC("desFire", RpcTarget.AllBuffered, fireEffect);
                //}
            ...
            {
                cooldown = false;
                skillBtn.transform.Find("Text").GetComponent<Text>().text = "";
            }
        }
    }
    ...
    {
        awardScreen = GameObject.Find("Canvas").transform.Find("AwardScreen").gameObject;
        awardScreen.GetComponent<Image>().sprite = mortWon;
