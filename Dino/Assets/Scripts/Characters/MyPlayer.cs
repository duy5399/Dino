...
public class MyPlayer : MonoBehaviourPun, IPunObservable
{
    ...
    public float jumpForce = 475f;   //độ nhảy
    ...
    void Start()
    {
        ...
        //    skillBtn = GameObject.Find("Canvas").transform.Find("ButtonSkill").gameObject;
        //}
        //else
        //{
    ...
    public void SmoothHealth()   //hàm cập nhập vị trí của người chơi khác khi smoothMove và smoothMoveRotation  được update vị trí mới từ hàm OnPhotonSerializeView();
    {
        blood.fillAmount = smoothHeal/maxHealth;    //khi nhân vận tốc với Time.delta thì object di chuyển ko phụ thuộc vào FPS của game
    }

    public void ProcessInputs()    //hàm điều khiển nhân vật di chuyển
    {
    ...
    {
        if (collision.gameObject.tag == "AKick" || collision.gameObject.tag == "Fire" || collision.gameObject.tag == "Ice")          //đối tượng va chạm là đạn
        {
        ...
        if (collision.gameObject.tag == "DeathZone" || collision.gameObject.tag == "Stone")          //đối tượng va chạm là đáy
        {
            if (photonView.IsMine)
            {
                musicSource[1].Play();
    ...
    {
        GameObject AKick;
        if (CrossPlatformInputManager.GetButtonDown("Kick") && !kicking && kickBtn.interactable == true)
        {
            if (sr.flipX == true)
            {
        ...
        if (kicking)
        {
            if(kickDelay > 0)
            {
                ...
                kickBtn.transform.Find("Text").GetComponent<Text>().text = ""+Math.Round(kickDelay, 1);
            }
            else
            {
                kicking = false;
                kickBtn.transform.Find("Text").GetComponent<Text>().text = "";
            }
        }
    ...
    public void UpdateScoreBoard()      //hàm cập nhật bảng điểm
    {
        ...
        var sortedPlayerList = (from p in PhotonNetwork.PlayerList orderby p.GetScore() descending select p).ToList();
        int i = 0;
        foreach (Player player in sortedPlayerList)    //lấy từng người chơi trong phòng
        {
            //append: chắp thêm
            i++;
            playerList.Append("#" + i +" " + player.NickName + "\t "  + player.GetScore() + "\n");    //lấy tên và điểm số của người chơi gán vào playerList (StringBuilder)
        }
    ...
    [PunRPC]
    public virtual void SetAward(string nameWinner)
    {
        
    }

    public virtual void Victory()       //hàm kiểm tra điểm số (người chiến thắng)
    {
