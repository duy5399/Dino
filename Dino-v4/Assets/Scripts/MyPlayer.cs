...
public class MyPlayer : MonoBehaviourPun, IPunObservable
{
    ...
    //public GameObject ramSpawnRight;   //vị trí Kick xuất hiện bên phải
    //public GameObject ramSpawnLeft;   //vị trí Kick xuất hiện bên trái
    ...
    void OnCollisionEnter2D(Collision2D collision)  //di chuyển trên mặt đất
    {
        ...
        if (collision.gameObject.tag == "Ram")          //đối tượng va chạm là đạn
        {
            if (photonView.IsMine)
            {
                musicSource[1].Play();
    ...
    public void Kick()
    {
        ...
        if (kicking)
        {
            ...
            {
                kicking = false;
                kickBtn.transform.Find("Text").GetComponent<Text>().text = "Kick";
            }
        }
    ...
    public void Skill()
    {
        ...
        if (CrossPlatformInputManager.GetButtonDown("Skill") && !cooldown)
        {
            Debug.Log("Skill");
